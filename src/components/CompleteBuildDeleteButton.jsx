...
export default function CompleteBuildDeleteButton({ buildId, refresh }) {
        ...
        <div className="fixed top-0 left-0 w-screen h-screen z-[99999] flex items-center justify-center bg-black/70 backdrop-blur-sm m-0 p-0">
            ...
            <div className="w-[420px] max-w-[90%] bg-gray-900/95 backdrop-blur-xl border border-white/10 rounded-2xl flex flex-col overflow-hidden shadow-2xl">
                ...
                <div className="p-6 flex flex-col items-center justify-center text-center text-gray-300 gap-3">
                    <p className="w-full break-words">Are you sure you want to delete the complete build with ID:</p>
                    <span className="text-white font-bold bg-white/10 px-4 py-1.5 rounded-md text-lg shadow-inner">{buildId}</span>
                ...
                <div className="w-full flex p-5 justify-end items-center gap-3 bg-white/5 border-t border-white/10">
                    ...
                    <button 
                        ...
                        onClick={() => {
                            const toastId = toast.loading("Deleting build...");
                            const token = localStorage.getItem("token");
                            
                            api.delete("/complete-builds/" + buildId, {
                                headers: {
                                    Authorization: `Bearer ${token}`,
                                },
                            }).then(() => {
                                toast.success("Complete Build deleted successfully!", { id: toastId });
                                refresh();
                                setIsModalVisible(false);
                            }).catch(() => {
                                toast.error("Error deleting build", { id: toastId });
                            });
                        }}
                    >
                        Delete
