...
export default function AdminEditCompleteBuildForm() {
    const location = useLocation();
    const buildData = location.state;

    const [buildId, setBuildId] = useState(buildData?.buildId || "");
    const [name, setName] = useState(buildData?.name || "");
    const [altNames, setAltNames] = useState(buildData?.altNames?.join(",") || "");
    const [description, setDescription] = useState(buildData?.description || "");
    const [price, setPrice] = useState(buildData?.price || "");
    const [labelledPrice, setLabelledPrice] = useState(buildData?.labelledPrice || "");
    const [images, setImages] = useState([]);
    const [isAvailable, setIsAvailable] = useState(buildData?.isAvailable ?? true);
    const [stock, setStock] = useState(buildData?.stock || 0);
    const [brand, setBrand] = useState(buildData?.brand || "");
    const [model, setModel] = useState(buildData?.model || "");
    
    // PC Specs
    const [cpu, setCpu] = useState(buildData?.cpu || "");
    const [gpu, setGpu] = useState(buildData?.gpu || "");
    const [motherboard, setMotherboard] = useState(buildData?.motherboard || "");
    const [ram, setRam] = useState(buildData?.ram || "");
    const [storage, setStorage] = useState(buildData?.storage || "");
    const [cpuCooler, setCpuCooler] = useState(buildData?.cpuCooler || "");
    const [powerSupply, setPowerSupply] = useState(buildData?.powerSupply || "");
    const [pcCase, setPcCase] = useState(buildData?.pcCase || "");
    const [operatingSystem, setOperatingSystem] = useState(buildData?.operatingSystem || "");
    const [wifiBluetooth, setWifiBluetooth] = useState(buildData?.wifiBluetooth || "");
    const [rgbSupport, setRgbSupport] = useState(buildData?.rgbSupport || "");
    ...
    async function updateBuild() {
        ...
        try {
            const imageUrls = images.length > 0 ? await Promise.all(imageUploadPromises) : buildData.images;
            const altNamesArray = altNames.split(",").map(name => name.trim()).filter(name => name);

            const requestBody = {
                name, altNames: altNamesArray, description, price, labelledPrice, images: imageUrls,
                isAvailable, stock, brand, model,
                cpu, gpu, motherboard, ram, storage, cpuCooler, powerSupply, pcCase, operatingSystem, wifiBluetooth, rgbSupport
            };

            await api.put("/complete-builds/" + buildId, requestBody, {
                headers: { Authorization: "Bearer " + token }
            });

            toast.success("Complete Build updated successfully");
            navigate("/admin/complete-builds");
            setIsLoading(false);
        } catch (error) {
    ...
    return (
        <div className="w-full h-full flex flex-col items-center p-4">
            ...
            <div className="w-full h-[90px] backdrop-blur-md bg-white/5 border border-white/10 shadow-lg rounded-xl flex px-6 items-center justify-between mb-8">
                <h1 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#FF2DA6] to-purple-400">Edit Complete Build: {buildId}</h1>
            ...
            <div className="w-full flex flex-wrap gap-y-6 backdrop-blur-sm bg-black/20 p-6 rounded-xl border border-white/5 shadow-2xl mb-12">
                ...
                <div className="w-full md:w-[32%] flex flex-col px-2">
                    <label className={labelClassName}>Build ID (Disabled)</label>
                    <input value={buildId} disabled className={`${inputClassName} opacity-50 cursor-not-allowed`} />
                </div>
                <div className="w-full md:w-[32%] flex flex-col px-2">
